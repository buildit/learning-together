...
class WorkshopList extends Component {
  constructor(props) {
    ...
    this.getWorkshopListPastCallback = this.getWorkshopListPastCallback.bind(
      this
    );
  }
  ...
  componentDidMount() {
    this.setState({
      isWorkshopListLoading: true,
      isWorkshopListPastLoading: true
    });
    getWorkshopList(
      this.props.computedMatch.params.id,
      this.getWorkshopListCallback
    );
    getWorkshopListPast(
      this.props.computedMatch.params.id,
      this.getWorkshopListPastCallback
    );
  }
  ...
  getWorkshopListCallback(response) {
    ...
    if (response.status === 200) {
      ...
      this.setState({
        error:
          "Could not retrieve workshop list at this time. Please try again later."
      });
    }
  }
  ...
  getWorkshopListPastCallback(response) {
    ...
    if (response.status === 200) {
      ...
      this.setState({
        error:
          "Could not retrieve previous workshops at this time. Please try again later"
      });
    }
  }
          ...
          <Tabs defaultIndex={0}>
            ...
            <TabPanel>
              {dates.map((date, index) => {
                return Object.keys(date).map((key, index) => {
                  return (
            ...
            <TabPanel>
              {workshops.map((workshop, index) => {
                const isVideo = workshop.archiveLink ? (
                  <article
                    className="workshopsforday"
                    key={`workshop-video-${index}`}
                  >
